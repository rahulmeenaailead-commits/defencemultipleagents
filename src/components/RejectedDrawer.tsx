...
const REASON_LABELS: Record<RejectedFinding["reason"], { label: string; tone: string }> = {
  TEXT_SPAN_NOT_FOUND: { label: "Quote not in clause", tone: "bg-orange-500/15 text-orange-300 border-orange-500/40" },
  CITATION_NOT_IN_CORPUS: { label: "Citation not in corpus", tone: "bg-amber-500/15 text-amber-300 border-amber-500/40" },
  SCHEMA_INVALID: { label: "Schema invalid", tone: "bg-red-500/15 text-red-300 border-red-500/40" },
};
    ...
    <section className="flex h-full flex-col overflow-hidden bg-[var(--color-bg)]">
      <header className="border-b border-slate-800 px-5 py-3">
        <h2 className="text-base font-semibold text-slate-100">
          Rejected drafts <span className="font-mono text-sm text-slate-500">· {rejected.length}</span>
        </h2>
