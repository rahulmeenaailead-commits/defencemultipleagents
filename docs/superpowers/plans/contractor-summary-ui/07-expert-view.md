# Task 7 — Extract ExpertView from ResultsView

**Files:**
- Create: `src/components/ExpertView.tsx`
- Modify: `src/components/ResultsView.tsx` *(not yet — happens in Task 8)*

Pure refactor — behavior must remain identical to today's `/analysis/[jobId]` body. After this task the app still works (`ResultsView` not yet rewired); only internal structure has moved. The pass-type filter (`PER_CLAUSE / CROSS_REF / HIDDEN`), which today sits in the TopBar of `ResultsView`, moves inside this `ExpertView` as an inner row of pills above the 3-column grid.

---

- [ ] **Step 1: Create the ExpertView with today's body**
...
`npm run typecheck` — no errors (ExpertView is not yet wired in; Task 8 does that).

- [ ] **Step 3: Commit**

```bash
git add src/components/ExpertView.tsx
git commit -m "refactor(ui): extract ExpertView from ResultsView (3-col + pass-type filter)"
```
