# Task 9 — RejectedDrawer: plain-English header, collapse raw JSON

**Files:** Modify `src/components/RejectedDrawer.tsx`

Today's `RejectedDrawer` dumps `JSON.stringify(r.rawDraft)` in a `<pre>` block as the primary content. New layout per the design spec: plain-English label at top (e.g. "Citation not in corpus"), the category, the quote the model claimed (extracted from `rawDraft.textSpan?.quote` when present), a one-line "why dropped" explanation. Raw JSON moves into a collapsed `<details>` under "Show raw model output".

Rejection reasons (`REASON_LABELS`) cover the three verifier failure modes: `TEXT_SPAN_NOT_FOUND`, `CITATION_NOT_IN_CORPUS`, `SCHEMA_INVALID`.

---

- [ ] **Step 1: Replace the file contents**

Replace the entire contents of `src/components/RejectedDrawer.tsx` with:
...
In the Technical tab, click the "Rejected by verifier · N" button at the bottom of `ClauseList`. Verify each entry shows the plain-English label, category, claimed quote (if extractable), and "Why dropped" sentence. Raw JSON is collapsed under "Show raw model output".

- [ ] **Step 4: Commit**

```bash
git add src/components/RejectedDrawer.tsx
git commit -m "feat(ui): rejected drafts — plain-English summary, raw JSON behind <details>"
```
