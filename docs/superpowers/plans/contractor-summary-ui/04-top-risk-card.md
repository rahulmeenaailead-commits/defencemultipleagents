# Task 4 — TopRiskCard component

**Files:** Create `src/components/TopRiskCard.tsx`

A single card in the Summary tab's top-risks list. Shows: severity dot + clause ID + severity label + category headline, the first sentence of `finding.reasoning`, an optional 💡 recommended-action chip, and a "See clause text →" button that calls `onOpen`.

Special badges:
- `Across §X and §Y` for `passType === "CROSS_REF"` with non-empty `relatedClauseIds`
- `🔍 Hidden risk` for `passType === "HIDDEN"`

Omit the action chip when `recommendedAction` is falsy.

---

- [ ] **Step 1: Implement the component**

Create `src/components/TopRiskCard.tsx`:
...
```bash
git add src/components/TopRiskCard.tsx
git commit -m "feat(ui): TopRiskCard — severity dot, plain-English headline, action chip"
```
